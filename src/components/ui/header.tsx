    ...
    <header className='w-screen px-2 py-4 z-50 flex flex-col items-center justify-center fixed bg-background drop-shadow-xl'>
      <div className='max-w-7xl w-full flex justify-between items-center'>
        <div className='w-64 cursor-pointer' onClick={() => router.push('/')}>
          <Image src={theme == 'light' ? '/autotech.png' : '/logo.png'} width={75} height={10} alt="Autotech" />
        </div>

        <Dialog onOpenChange={changeOpenSearch} open={openSearch}>
          <DialogTrigger asChild>
            <Button variant='outline' onClick={changeOpenSearch} className='w-96 rounded-full text-start flex items-start justify-start'>Busque um veículo</Button>
          </DialogTrigger>
        ...
        <nav className='flex gap-4 items-center w-64'>
          { loading ? (
            ...
            <>
              <Dialog>
                <DialogTrigger asChild>
                  <Button className='rounded-full px-5' variant='outline'>CADASTRAR</Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
              ...
              <Dialog onOpenChange={changeOpenLogin} open={openLogin}>
                <DialogTrigger asChild>
                  <Button className='rounded-full px-5' onClick={changeOpenLogin}>ENTRAR</Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    <DialogTitle>Faça seu login</DialogTitle>
                    <DialogDescription>
                      Informe seu email e sua senha para aproveitar nossas funcionalidades.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
      ...
      <div className='max-w-7xl w-full flex justify-between items-center pr-5'>
        <nav className='flex gap-4 items-center'>
          <Dialog onOpenChange={changeOpenSolicitationAdds} open={openSolicitationAdds}>
            <DialogTrigger asChild>
              <Button className='rounded-full px-5' variant='link' onChange={changeOpenSolicitationAdds}>Venda ou alugue seu veículo</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
              ...
              <div className="grid gap-4 py-4">
                <SolicitationAddsForm closeSolicitationAdds={changeOpenSolicitationAdds} />
              </div>
            </DialogContent>
          </Dialog>
          <Button className='rounded-full px-5' variant='link'>Alugue um veículo</Button>
          <Button className='rounded-full px-5' variant='link'>Reserve um veículo</Button>
        </nav>
